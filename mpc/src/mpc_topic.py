...
class MPC(Node):
    ...
    def mpc(self):
        ...
        if i+Np > len(self.path):
            additional_ind = (i+Np)-len(self.path)
            U_ref_ = np.vstack((U_ref[i:,:],U_ref[1:additional_ind+1,:]))
        else:
            U_ref_ = U_ref[i:i+Np,:]

        
        self.vel, self.omega = mpc_controller(i, A, B, C, w, init, Np, U_ref_, Q, R, U_min, U_max)
